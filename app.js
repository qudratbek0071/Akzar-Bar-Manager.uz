let API_BASE = localStorage.getItem("apiUrl") || "http://localhost:5500";
...
async function loadTables(){ const floor = state.currentFloor; const rows = await api(`/orders/tables/${floor}`); state.tables = rows; renderTables(); await refreshOrdersSummary(); }
...
// Reports
async function loadReports(){ try{ const daily = await api('/reports/daily'); const today = daily[0] || { total_sales:0, unpaid_total:0 }; ui.statSales.textContent=String(today.total_sales||0); ui.statUnpaid.textContent=String(today.unpaid_total||0); }catch(e){ showToast('Отчёт: '+e.message); }
  try{ const stock = await api('/stock/current'); ui.statLow.textContent=String(stock.lowStock.length); ui.stockBody.innerHTML = stock.stock.map(r=>`<tr><td>${r.name}</td><td>${r.category}</td><td>${r.stock}</td><td>${r.sold||0}</td>${state.role==='admin'?`<td><button class='btn btn-secondary btn-add-stock' data-id='${r.id}'>+Приход</button></td>`:''}</tr>`).join(''); stock.lowStock.forEach(i=> showToast(`${i.name}: мало (${i.stock})`));
    if (state.role==='admin') document.querySelectorAll('.btn-add-stock').forEach(b=> b.addEventListener('click', async (e)=>{
      const pid = Number(e.currentTarget.getAttribute('data-id'));
      const qty = Number(prompt('Кол-во прихода', '1'))||0; if (!qty) return;
      try{ await api('/stock', { method:'POST', body: JSON.stringify({ product_id: pid, qty, type:'incoming' })}); showToast('Приход добавлен'); await loadReports(); await loadStockEntries(); }catch(err){ showToast(err.message); }
    }));
  }catch(e){ showToast('Остатки: '+e.message); }
  try{ const borrowed = await api('/reports/borrowed'); ui.borrowBody.innerHTML = borrowed.map(b=>`<tr><td>${b.user_name}</td><td>${b.product_name}</td><td>${b.qty}</td><td>${b.returned_qty}</td><td>${b.outstanding}</td></tr>`).join(''); }catch{}
  try{
    const orders = await api('/reports/unpaid');
    ui.history.innerHTML = orders.map(o=>`<div class="list-item">${o.floor.toUpperCase()} Стол ${o.table_number} — ${o.total} (${o.paid?'оплачено':'не оплачено'}) ${state.role==='admin'?`<button data-id='${o.id}' class='btn btn-ghost btn-del'>Удалить</button>`:''}</div>`).join('');
    if (state.role==='admin') document.querySelectorAll('.btn-del').forEach(b=> b.addEventListener('click', async (e)=>{
      try{ const id=e.currentTarget.getAttribute('data-id'); await api(`/orders/${id}`, { method:'DELETE' }); showToast('Удалено'); await Promise.all([loadReports(), loadTables(), loadLeaderboard()]); }catch(err){ showToast(err.message); }
    }));
